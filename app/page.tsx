...
const models = [
  { id: "ltx-2.3", label: "LTX 2.3", note: "Fast" },
...
type HistoryItem = { id: string; prompt: string; url: string; createdAt: string; aspectRatio: string; style: string };
  ...
  async function waitForVideo(id: string) {
    ...
    while (Date.now() - startedAt < maxWait) {
      ...
      if (data.status === "complete" && data.url) {
        ...
        setStatus("done");
        saveHistory({ id, prompt: prompt.trim(), url: data.url, createdAt: new Date().toISOString(), aspectRatio, style });
        return;
      }
  ...
  async function generate() {
    ...
    try {
      const response = await fetch("/api/generate", {
        ...
        body: JSON.stringify({ prompt: value, model, aspectRatio, style, audio }),
      });
      const data = await response.json();
      ...
      <section className="hero" id="studio">
        ...
        <div className="composer">
          ...
          <div className="controls">
            ...
            <button type="button" className={`toggle ${audio ? "on" : ""}`} onClick={() => setAudio(!audio)} disabled={status === "generating"}><span>{audio ? "●" : "○"}</span> Audio</button>
          </div>
          <div className="composer-bottom"><div className="chips"><span>{aspectRatio}</span><span>5s</span><span>{style}</span><span>480p</span></div><button className="generate" onClick={generate} disabled={!prompt.trim() || status === "generating"}>{status === "generating" ? "Creating…" : "Generate"} <span>{status === "generating" ? "◌" : "↗"}</span></button></div>
        </div>
