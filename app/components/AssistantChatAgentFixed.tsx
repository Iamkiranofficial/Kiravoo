...
export default function AssistantChatAgentFixed() {
  ...
  async function handleTool(response: any, session: LiveSession) {
  ...
  async function connect() {
    ...
    try {
      ...
      const session = await ai.live.connect({
        ...
        callbacks: {
          ...
          onmessage: async (msg: any) => {
            if (msg?.toolCall) { await handleTool(msg, session); return; }
          ...
          onerror: (e: any) => {
            setConnecting(false);
            setLive(false);
            setError(e?.message || "Gemini Live connection failed.");
          },
          onclose: (e: any) => {
            setConnecting(false);
            setLive(false);
      ...
      proc.onaudioprocess = e => {
        ...
        try {
          session.sendRealtimeInput({ audio: { data: pcmB64(downsample(e.inputBuffer.getChannelData(0), inputCtx.sampleRate)), mimeType: "audio/pcm;rate=16000" } });
        } catch {}
      };
      source.connect(proc);
      proc.connect(gain);
  ...
  async function sendText() {
    ...
    if (live && sessionRef.current) {
      try { sessionRef.current.sendRealtimeInput({ text }); } catch (e) { setError(e instanceof Error ? e.message : "Could not send message."); }
      return;
    }
    setBusy(true);
    try {
