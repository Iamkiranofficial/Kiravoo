...
export default function CreatorNav(){
 ...
 return <nav aria-label="KIRAVO workspace navigation" className="creator-nav"><div className="creator-nav-scroll">{items.map(([label,href])=>{
  const active=pathname==="/" ? (label==="Studio" ? !workspace : label.toLowerCase()===workspace) : (href.split("?")[0]!=="/" && pathname.startsWith(href.split("?")[0]));
  return <button key={label} type="button" className={active?"active":""} aria-current={active?"page":undefined} onClick={()=>router.push(href)}><span>{icons[label]}</span><b>{label}</b></button>;
 })}</div><style jsx>{`
